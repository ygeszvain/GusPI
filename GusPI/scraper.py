...
def YelpBizInfo(CUISINES):
    ...
    for name in CUISINES:
        ...
        for cp in np.arange(0, 1):
            ...
            biz.append(name)
            aggregateRating.extend(i for i in [meta_aggregateRating.attrs['content'] for meta_aggregateRating in
                                               metas_aggregateRating[0:1] if 'itemprop' in meta_aggregateRating.attrs])
            priceRange.extend(i for i in [meta_priceRange.attrs['content'] for meta_priceRange in metas_priceRange if
                                          'itemprop' in meta_priceRange.attrs])
    ...
    yelp = pd.concat([biz, aggregateRating, priceRange, streetAddress, addressLocality, addressRegion, postalCode],
                     axis=1, ignore_index=True)
    yelp.columns = ['biz', 'aggregateRating', 'priceRange', 'streetAddress', 'addressLocality', 'addressRegion',
                    'postalCode']
...
def YelpReview(CUISINES):
    for name in CUISINES:
        ...
        for cp in np.arange(0, 10):
            ...
            date.extend(
                i for i in [meta_date.attrs['content'] for meta_date in metas_date if 'itemprop' in meta_date.attrs])
            author.extend(i for i in [meta_author.attrs['content'] for meta_author in metas_author if
                                      'itemprop' in meta_author.attrs])
